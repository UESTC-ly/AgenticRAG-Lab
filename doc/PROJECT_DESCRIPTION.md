...
- 检索层同时保留规则版 baseline 与真实模型实现 —— 当前源码默认 real path 使用 `BAAI/bge-m3` + `BAAI/bge-reranker-v2-m3`; benchmark 文档另收录 `bge-small-en-v1.5` + `bge-reranker-base` 的实验结果
- 生成层有 `LLMSynthesizer` 通过本地 Ollama 调用 `qwen2.5:7b`(默认)或其他本地模型,并带规则版降级
- 评测层已有 HotpotQA 500 样本 retrieval 消融、100 样本 reranker 验证和 20 样本端到端 benchmark(F1: 0.085 → 0.434)
...
- 真实 BM25（当前 lexical 仍是 BM25-like token / idf 打分）
...
虽然真实模型栈已经落地,源码默认 real path 为 `BAAI/bge-m3` + `BAAI/bge-reranker-v2-m3` + `qwen2.5:7b`; benchmark 文档中也记录了 `bge-small-en-v1.5` + `bge-reranker-base` + `gemma4:e2b` 的实验结果。当前 retrieval 在 500 样本无 reranker 实验里 `semantic-bge @5 = 0.798`,在 100 样本含 reranker 验证里 `hybrid-bge-rerank @5 = 0.930`,端到端 F1 从 0.085 提升到 0.434,短板也随之转移。
...
- ✅ `EmbeddingSemanticRetriever` 已接入 `sentence-transformers`(源码默认 `BAAI/bge-m3`)
- ✅ `CrossEncoderReranker` 已接入 cross-encoder rerank(源码默认 `BAAI/bge-reranker-v2-m3`)
- ✅ `LLMSynthesizer` 已通过本地 Ollama 接入(源码默认 `qwen2.5:7b`)
- ✅ 三者均支持独立降级,测试通过 fake-model 注入保持 CI 无 GPU 也能跑
- ✅ HotpotQA retrieval 已有 500 样本无 reranker 与 100 样本含 reranker 两组结果；`all_supporting_docs_hit_rate@5` 分别达到 `0.798`(semantic-bge,500) 与 `0.930`(hybrid-bge-rerank,100)
...
- 有问题定义(多跳问答的失败模式)
- 有系统设计(手写状态机,每一跳可解释)
- 有真实模型栈(源码默认 `bge-m3 + bge-reranker-v2-m3 + qwen2.5:7b`,并保留历史 benchmark 配置记录)
- 有 benchmark(500 样本 retrieval + 20 样本 E2E,含 6 配置 ablation)
- 有产品原型(Web UI + 多知识库 + 文档导入)
- 有测试(51 个单元测试,fake-model 注入保证无 GPU 也能跑)
...
> 一个已经打通 **数据接入 → 检索评测(BM25-like lexical + dense + reranker) → Agentic RAG 主链路 → LLM 生成 → 本地可用产品工作台** 的 Agentic RAG 项目,并在 HotpotQA 上完成 6 配置 retrieval ablation + 4 配置端到端 benchmark。

...
flowchart TD
    ...
    RETRIEVER --> DENSE[Embedding Semantic Retriever<br/>default: bge-m3]
    RETRIEVER --> HYB[Hybrid Fusion / RRF]
    HYB --> RER[Cross-Encoder Reranker<br/>default: bge-reranker-v2-m3]
    SYNTH --> LLMSYN[LLM Synthesizer<br/>Ollama / qwen2.5:7b default]
...
- ✅ dense embedding:`EmbeddingSemanticRetriever` (源码默认 `BAAI/bge-m3`; benchmark 文档另收录 `bge-small-en-v1.5` 实验)
- ✅ cross-encoder reranker:`CrossEncoderReranker` (源码默认 `BAAI/bge-reranker-v2-m3`; benchmark 文档另收录 `bge-reranker-base` 实验)
- ✅ LLM synthesizer:`LLMSynthesizer` 走 Ollama / `qwen2.5:7b`(默认,可覆盖)
- ✅ 三组件独立降级:任一失败自动退回规则版
...
当前 `SemanticRetriever` 规则版保留作 baseline;源码默认 real path 已切到 `BAAI/bge-m3` + `BAAI/bge-reranker-v2-m3`。`doc/BENCHMARK_RESULTS.md` 记录的是更早一组 `bge-small-en-v1.5` + `bge-reranker-base` 的实验结果。后续仍可接入真实 `rank_bm25` 或继续调大模型规格。
...
BAAI/bge-m3 + BAAI/bge-reranker-v2-m3
   ↓
benchmark 文档已收录 500 样本无 reranker / 100 样本含 reranker 结果
```

完整结果见 `doc/BENCHMARK_RESULTS.md`。

### 11.2 [已接入,待扩大评测] 真实 LLM Synthesizer

```text
Rule-based Synthesizer
   ↓
LLMSynthesizer + Ollama (default: qwen2.5:7b)       ← 已接入
   ↓
端到端 EM / F1 / citation rate benchmark            ← 已有 20 样本结果,待扩大
```
...
> 一个由 `Agentic RAG` 主链路驱动、以 `HotpotQA` 检索评测与本地多知识库 MVP 为双底座、默认 real path 为 `bge-m3 + bge-reranker-v2-m3 + qwen2.5:7b` 的可演进系统原型。

如果从面试角度讲,这套架构已经支撑**「系统设计 + 真实模型消融 + 产品化」三条线同时展开**,并且有具体数字可讲(例如 500 样本 `semantic-bge @5 = 0.798`,100 样本 `hybrid-bge-rerank @5 = 0.930`)。
